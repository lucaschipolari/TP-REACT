...
const Ejercicio11View = () => {
    ...
    const fetchNoticiasPorCategoria = async () => {
        try {
            const response = await fetch(`https://newsapi.org/v2/top-headlines?category=${categoriaSeleccionada}&apiKey=YOUR_API_KEY`);
            const data = await response.json();
    ...
    return (
        <div>
            <FormularioBusqueda noticias={noticias} setCategoriaSeleccionada={setCategoriaSeleccionada} />
            {noticias.length === 0 ? <h1>Cargando...</h1> : <ListadoNoticias noticias={noticias} />}
        </div>
    );
};
